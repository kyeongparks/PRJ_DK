...
public class UIManager : MonoBehaviour
{
    ...
    protected T CreateMissingLayer<T>(UILayerTypes type, string name) where T : Component
    {
        ...
        UIUtils.SetParent(go.transform, root);
        return comp;
    }
    ...
    public AsyncOperation LoadLevelAsync(string levelName, string viewName, object param)
    {
        initialViewName = viewName;
        viewParam = param;
    ...
    {
        //LoadingPopup Àº Á¦¿Ü
        List<UILayer> acivePopups = CurrentPopups.FindAll(p => p.gameObject.activeInHierarchy);
